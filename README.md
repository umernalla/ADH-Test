...
EMA, RFA and be familiar with consuming OMM Market Price data.
...
### Prerequisites ###
In order to use the above feature, you will off course need a TREP installation and the ability to amend / update the TREP configuration. If you are unsure please speak first to your Market Data team to ensure this is possible.
...
For EMA Java you can use the 
```
XmlTraceToStdout
``` 
parameter with standard Java functionality to redirect StdOut to a file.
...
Posting allows Consumer / desktop applications to push content into the ADH Cache. The application needs to Login to server as per normal and then use that session to Push the content to the server. There are two types of Posting mechanism that can be used - Off-Stream and On-Stream:    
...
So, for example if  you have a desktop application where the user views an existing item and then perhaps wants to update it, you would use On-Stream Posting. If, however, the user just wishes to create new item or update an existing item based on data from another source, you could use Off-Stream Posting.   
...
For this I will use the example341 from the EMA devkit as the starting point.    
The following explanation assumes you are familiar with using EMA to consumer MarketPrice data as a minimum - as per the prerequisites. 

Normally, EMA performs the the Login request and processes the response on behalf of the developer. However, to perform an Off-Stream post the application needs to get hold of the handle for the Login stream so it can be used to submit the PostMsg.
...
As you may know, whatever you pass in the Closure parameter is made available to the callback methods. 
...
- the method first verifies that it has received a Login Refresh
...
- Finally the PostMsg is submitted as follows:  
  ...
  *Finally the RefreshMsg is decoded and dumped out to the console     
...
Example output from the onRefreshMsg and the decode() method call:

```

Received Refresh. Item Handle: 1 Closure: com.thomsonreuters.ema.access.OmmConsumerImpl@668dc22c  
Item Name: umer.nalla  
Service Name: <not set>  
Item State: Open / Ok / None / 'Login accepted by host ads1-a'  
 ...
 Name = Position DataType: Ascii Value: 10.14.33.126/DESKTOP-DN44M  
 ...
 Name = SupportPauseResume DataType: UInt Value: 1  
 ...
 Name = SupportOptimizedPauseResume DataType: UInt Value: 1  
```

A few things to note, the application is using a RefreshMsg (for the payload of the PostMsg) as it wants to send a full Refresh specifying all the fields the Records will contain. If, however the record already exists and only a subset of the fields need to be updated, then an UpdateMsg payload should be used instead.   
...
As you can see the Bid, Ask, Trade Time and Dividend Type field have been populate as per the FieldList values specified in the onRefreshMsg() method earlier.   
...
The TEST.TST record no longer exists and the Stream State and Data State are reported as CLOSED and SUSPECT respectively. The StatusCode has been set to TIMEOUT and the StatusText reports 'F67: Auto-expiry Item Timeout'

So, as you can see the record was expired after five minutes and removed from the ADH Cache.
